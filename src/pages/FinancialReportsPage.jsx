...
import { Download, BookOpen, TrendingUp, Wallet, FileText, Plus, X, Edit2, Trash2, AlertTriangle } from 'lucide-react';
...
const FinancialReportsPage = () => {
  ...
  // Load initial report data on mount
  useEffect(() => {
    if (activeTab !== 'jurnal') {
      loadReportData();
    }
  ...
  useEffect(() => {
    const handleExternalTransactionUpdate = () => {
      if (activeTab === 'jurnal') {
        loadJournalEntries();
      } else {
        loadReportData();
    ...
    window.addEventListener('financial-transactions-updated', handleExternalTransactionUpdate);
    return () => window.removeEventListener('financial-transactions-updated', handleExternalTransactionUpdate);
  }, [activeTab, selectedBranch, startDate, endDate, selectedAccount, selectedRekening, currentPage, itemsPerPage]);
  ...
  const reportFilters = useMemo(() => {
        ...
        render: ({ value, onChange }) => (
          ...
          >
            ...
            {(accounts || []).map(account => (
              <option key={account.id_akun} value={account.kode_akun || account.id_akun}>
    ...
    if (activeTab === 'arus-kas' || activeTab === 'kas') {
      filters.push({
        key: 'id_rekening',
        label: 'Rekening',
        render: ({ value, onChange }) => (
          ...
          >
            ...
            {(financialAccounts || []).map(rekening => (
              <option key={rekening.id_rekening} value={rekening.id_rekening}>
    ...
    return filters;
  }, [activeTab, branches, accounts, financialAccounts]);
  ...
  const handleDataTableFilterChange = ({ filterValues }) => {
    setSelectedBranch(filterValues.id_cabang || '');
    setStartDate(filterValues.tanggal_dari || '');
    setEndDate(filterValues.tanggal_sampai || '');
    setSelectedAccount(filterValues.id_akun || '');
    setSelectedRekening(filterValues.id_rekening || '');
    setCurrentPage(1);
  };
  ...
  const loadReportData = async () => {
    ...
    try {
      ...
      console.error('Failed to load report data:', err?.response?.data || err);
  ...
  const EmptyState = ({ title = 'Tidak Ada Data', message = 'Data tidak ditemukan untuk filter yang dipilih' }) => (
    ...
    switch (activeTab) {
      case 'buku-besar':
        if (!data.rows || !Array.isArray(data.rows) || data.rows.length === 0) {
          return <EmptyState title="Buku Besar Kosong" message="Tidak ada transaksi untuk periode ini" />;
        }
        return (
          <DataTable
            data={data.rows}
      ...
      case 'arus-kas':
        if (!data.items || !Array.isArray(data.items) || data.items.length === 0) {
          return <EmptyState title="Arus Kas Kosong" message="Tidak ada data arus kas untuk periode ini" />;
        }
        return (
          <DataTable
            data={data.items}
      ...
      case 'kas':
        if (!data.rekening || !Array.isArray(data.rekening) || data.rekening.length === 0) {
          return <EmptyState title="Kas Kosong" message="Tidak ada data kas untuk periode ini" />;
        }
        return (
          <DataTable
            data={data.rekening}
      ...
      case 'neraca':
        if (!data.neraca || !Array.isArray(data.neraca) || data.neraca.length === 0) {
          return <EmptyState title="Neraca Kosong" message="Tidak ada data neraca untuk periode ini" />;
        }
        return (
          <div className="space-y-6">
            {data.neraca.map((category, index) => (
              <div key={index} className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
                ...
                {(!category.accounts || !Array.isArray(category.accounts) || category.accounts.length === 0) ? (
                  <EmptyState title="Tidak Ada Akun" message={`Tidak ada akun untuk kategori ${category.tipe_akun}`} />
                ) : (
                  <DataTable
    ...
    return (
      <DataTable
        ...
        columns={[
          ...
          { key: 'keterangan', header: 'Keterangan' },
          { key: 'cabang', header: 'Cabang', render: (row) => row.cabang?.nama_cabang || '-' },
          { key: 'total_debit', header: 'Debit', render: (row) => formatCurrency(row.total_debit), className: 'text-right' },
          { key: 'total_kredit', header: 'Kredit', render: (row) => formatCurrency(row.total_kredit), className: 'text-right' },
          {
            key: 'actions',
            header: 'Aksi',
            render: (row) => (
              <div className="flex justify-center gap-2">
                <button
                  onClick={() => handleOpenJournalForm(row)}
                ...
                <button
                  onClick={() => handleDeleteJournal(row.id_jurnal)}
        ...
        searchKeys={['keterangan', 'cabang.nama_cabang']}
      />
    );
  };
  ...
  return (
    <PageLayout>
      <PageContainer>
        ...
        {/* Summary Cards - Temporarily removed to fix error */}
        {/* {!loading && !error && activeTab !== 'jurnal' && data && renderSummaryCards()} */}

        {/* Report Table */}
        {!loading && !error && activeTab !== 'jurnal' && (data ? renderReportTable() : <EmptyState title="Tidak Ada Data" message="Gunakan kotak pencarian dan filter tabel untuk menampilkan data laporan" />)}

        {/* Journal Table with Pagination */}
        {!journalLoading && activeTab === 'jurnal' && (
          <>
            {journalEntries.length === 0 ? (
              <EmptyState title="Jurnal Kosong" message="Tidak ada jurnal yang dibuat. Mulai dengan membuat jurnal baru." />
            ) : (
              <>
                {renderJournalTable()}
